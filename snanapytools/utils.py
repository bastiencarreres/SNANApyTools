...
@guvectorize(
    ["void(float64[:, :], float64[:, :], float64[:,:])"], "(m, n),(m, n)->(m, n)", nopython=True
)
def new_coord_on_fields(ra_dec, ra_dec_frame, new_radec):
    ...
    for i in range(len(ra_dec_frame[0])):
        vec = np.array(
            [
                np.cos(ra_dec[0][i]) * np.cos(ra_dec[1][i]),
...
def _compute_polygon(corners):
    ...
    if polygon.intersects(_SPHERE_LIMIT_):
        ...
        polygon = [
            p for p in shp_ops.polygonize(unioned) if p.representative_point().within(polygon)
        ]
...
def format_poly(poly):
    ...
    """
    _SPHERE_LIMIT_HIGH_ = shp_geo.LineString([[2 * np.pi, -np.pi / 2], [2 * np.pi, np.pi / 2]])
