...
def write_pdf(path, par_dic, docstr="DOCUMENTATION:\nDOCUMENTATION_END:", format_dic={}):
    file = open(path, "w")
    file.write(docstr + "\n\n")

    for k in par_dic:
        file.write("VARNAMES: " + " ".join([v for v in par_dic[k].keys()]) + "\n")
        ...
        lines = "\n".join([line.format(*values) for values in zip(*par_dic[k].values())])
        file.write(lines)
        file.write("\n\n")
    file.close()
...
def apply_selec_pippin_dir(
    ...
    for seed, sd, fd in zip(Seeds, SIM_DIRS, FIT_DIRS):
        ...
        shutil.copy2(
            list(fd.glob("output/SUBMIT.INFO"))[0], VERSION_FIT_OUT_DIR / "output/SUBMIT.INFO"
        )
        ...
        for hf, pf in zip(head_files, phot_files):
            ...
            head_df = Table(header[1].data)[
                ["SNID", "PTROBS_MIN", "PTROBS_MAX", "NOBS", "SIM_REDSHIFT_CMB"]
            ].to_pandas()
