...
class PIPPIN_READER:
    ...
    def __init__(self, name: str, pippin_output: str=None):
        ...
        if pippin_output is None:
            ...
            self.__PIPPIN__OUTPUT__ = Path(pippin_output)
        ...
        for i, s in enumerate(self._STEPS):
            ...
            if path.exists():
                ...
                setattr(
                    self,
                    "available_" + s,
                    set(d.name for d in getattr(self, s + "_path").iterdir()),
                )
            else:
    ...
    def build_tree(self):
        ...
        for biascor_name in self.available_biascor:
            ...
            biascor_tree[biascor_name] = {
                "files": set(
                    d for d in biascor_dir.iterdir() if (d.is_dir() and "OUTPUT_BBCFIT" in d.name)
                )
            }

            biascor_summary, biascor_files = tls.read_biascor_input_doc(
                self.biascor_path / biascor_name / f"{biascor_name}.input"
            )
    ...
    def get_sim(self, sim_name: str | PosixPath, **kwargs) -> SNANA_SIM:
        ...
        if sim_name not in self.available_sim:
            raise ValueError(
                f"{sim_name} is not available. Check self.available_sim. Maybe refresh with self.up_dir('sim')"
            )
        sim_dir = self.sim_path / sim_name
    ...
    def get_biascor(self, biascor_name: str | PosixPath, **kwargs) -> SNANA_BIASCOR:
        ...
        if biascor_name not in self.available_biascor:
            raise ValueError(
                f"{biascor_name} is not available. Check self.available_biascor. Maybe refresh with self.up_dir('biascor')"
            )
        biascor_dir = self.biascor_path / biascor_name / "output"
        biascor_dir = list(
            d for d in biascor_dir.iterdir() if (d.is_dir() and "OUTPUT_BBCFIT" in d.name)
